...
// Constants for reuse
const BUTTON_BASE_CLASSES = "rounded-lg py-2.5 px-4 text-sm font-medium w-full flex justify-center items-center gap-2";
...
const QuestionIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-gray-300 mb-4 mx-auto" viewBox="0 0 20 20" fill="currentColor">
...
export default function ClassroomTestExportTab() {
  ...
  const [selectedDataset, setSelectedDataset] = useState<DatasetOption>('reserved');
  ...
  const WorkflowStep = ({ step, isActive, isCompleted, label }: { step: number; isActive: boolean; isCompleted: boolean; label: string }) => (
    <div className="flex items-center">
      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isActive || isCompleted ? 'bg-black text-white' : 'bg-[#E6E7E9] text-gray-500'}`}>
        {isCompleted ? <CheckmarkIcon /> : <span className="text-sm font-bold">{step}</span>}
      </div>
      <span className={`ml-2 text-sm font-medium ${isActive ? '' : 'text-gray-500'}`}>{label}</span>
    </div>
  );
  ...
  const ExportButton = ({ onClick, icon, label, isPrimary = false }: { onClick?: () => void; icon: ReactNode; label: string; isPrimary?: boolean }) => (
    <button 
      onClick={onClick}
    ...
    <div className="grid grid-cols-1 gap-6">
      ...
      <div className="flex items-center justify-center w-full mb-2">
        <div className="flex items-center space-x-2 md:space-x-4 bg-white px-6 py-3 rounded-xl shadow-sm">
          <WorkflowStep step={1} isActive={false} isCompleted={true} label="Data Input" />
          <div className="w-8 h-0.5 bg-black"></div>
          <WorkflowStep step={2} isActive={false} isCompleted={true} label="Model Selection" />
          <div className="w-8 h-0.5 bg-black"></div>
          <WorkflowStep step={3} isActive={false} isCompleted={true} label="Train & Validate" />
          <div className="w-8 h-0.5 bg-black"></div>
          <WorkflowStep step={4} isActive={true} isCompleted={false} label="Test & Export" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-sm mb-4">
                Select a test dataset to evaluate your model's performance on unseen data. This step
        ...
        <div className="lg:col-span-6">
          <Card>
            ...
            <CardContent>
              <p className="text-sm mb-4">
                These metrics show how well your model performs on data it hasn't seen during training.
              </p>
                ...
                <div className="flex flex-col items-center justify-center py-20">
                  {isTesting ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-sm mt-2">Run a test on your model to see performance metrics.</p>
                    </div>
                  )}
                </div>
              )}
      ...
      <div className="flex justify-between">
        ...
        <div className="fixed bottom-8 left-8 z-10">
          ...
          >
            <svg className="w-7 h-7 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
